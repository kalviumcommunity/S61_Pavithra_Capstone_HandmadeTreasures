...
function ProductsPage() {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:3000/api/read') // Corrected endpoint
            .then(response => {
                const adminProducts = response.data.products.filter(product => product.admin === adminId);
                setProducts(adminProducts); // Set only the products added by the current admin
                const uniqueCategories = adminProducts.reduce((acc, product) => {
                    if (!acc.includes(product.category)) {
                        acc.push(product.category);
                    }
                    return acc;
    ...
    return (
        <div className='pro'>
            <div className="product-list-container">
                <h1 className="product-list-title">My Products</h1>
                <div className="filter-area">
                    <select
                        id="categoryFilter"
                        value={filteredCategory}
